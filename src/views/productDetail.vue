...
const addToCart = async () => {
  ...
  // Get seller info
  const sellerDocSnap = await getDoc(doc(db, 'users', product.value.sellerId))
  const sellerDocData = sellerDocSnap.exists() ? sellerDocSnap.data() : {}
  const sellerName = sellerDocData.name || 'Unknown Seller'
  const sellerUsername = sellerDocData.username || product.value.sellerUsername || 'unknown'
  const sellerAvatar = sellerDocData.avatarUrl || ''
