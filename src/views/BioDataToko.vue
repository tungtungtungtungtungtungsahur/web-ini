<template>
  <div class="Biodata-product-container">
    ...
    <div class="form-wrapper">
      ...
      <div
        v-if="showJamOperasionalModal"
        class="modal-overlay"
        @click.self="showJamOperasionalModal = false"
      >
        <div class="modal-select modal-price">
    ...
    onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (tokoSnap.exists()) {
          ...
          this.description = tokoData.deskripsi || ''
          this.selectedLokasiDisplay = tokoData.lokasi || null
          this.selectedKategori = tokoData.kategori || []
          this.jamOperasional = tokoData.jamOperasional || ''
          this.kontak = tokoData.kontak || null
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        if (user) {
          const tokoRef = doc(db, 'toko', user.uid) // Use 'toko' collection
          await setDoc(
            tokoRef,
            {
              deskripsi: this.description,
              lokasi: this.selectedLokasiDisplay,
